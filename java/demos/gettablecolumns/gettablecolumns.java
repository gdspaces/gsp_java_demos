/**
 * @deprecated since v1.6.0.1, please check TGetTableColumn.java and runGetTableColumn instead.
*/
 ...
 *<p>3. Once a table was found, get all columns belong to this table via {@link gudusoft.gsqlparser.nodes.TTable#getLinkedColumns()} 
 */

class metaDB implements IMetaDatabase {

    String columns[][] = {
        {"server","db","dbo","subselect3table1","f1"},
        {"server","db","dbo","subselect3table2","s3t1a1"}
    };
        ...
        for(int i=0;i<stmt.tables.size();i++){
            if (stmt.tables.getTable(i).isBaseTable())
            {
              ...
              for (int j=0;j<stmt.tables.getTable(i).getLinkedColumns().size();j++){
                ...
                foundColumns[foundColumnsCount] += "(table determined:"+stmt.tables.getTable(i).getLinkedColumns().getObjectName(j).isTableDetermined()+")";
                  foundColumnsCount++;
              }
            }
